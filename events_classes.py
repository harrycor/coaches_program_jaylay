...
class EventsData:
    ...
    def event_info_create(self, data_body):
        ...
        for stripper_loop in data_body:
            stripper_loop = stripper_loop.text.strip("\n")  # REASON - no href when 'stripped'-**must strip to iso event
            try:
                int(stripper_loop)
            except ValueError:
        ...
        return event_date_name_local

    def data_dict_create(self, event_data, url_data):
        ...
        event_placeholder = []    # placeholder to load each events info before adding to dict. emptied each loop around
        marker_event = 0        # acts as if router
        marker_url = 0
        for create in event_data:
            if marker_event <= 2:
                # print(create)
                # print("*************")
                event_placeholder.append(create)    # adds event-title, date, location to placeholder
                if marker_event == 2:
                    if "’" in event_placeholder[2]: # this removes the "’" for the JSON conversion. else its "u2019".
                        # print("HELLO*******")
                        apos_remover = event_placeholder[2].replace("’", "\'")
                        event_placeholder[2] = apos_remover
                        # print(apos_remover)
                    try:
                        event_placeholder.append(url_data[marker_url])   # adds event URL to placeholder
        ...
        return data_dict_for_json

    def json_create(self, dict_for_json):
        # print(dict_for_json)
        # JSON used from dictionary data
        json_event = json.dumps(dict_for_json, indent=4)
        # print(json_event)
        # print("all good")
        return json_event
